...
class Schema:
  def __init__(self, database: str, name: str, target_database: str, target_schema: str ):
    self.name: str = name
...
def find_or_create_schema(schemas: List[Schema], database: str, schema_name: str, target_database: str, target_schema: str) -> Schema:
    for schema in schemas:
        if schema.name == schema_name and schema.database == database:
            return schema
    # If not found, create a new Schema and add it to the list
    new_schema = Schema(database, schema_name, target_database, target_schema)
...
def get_schema_data_config(pg_database, environment): 
  csv_file_path = f"{config.outputdir.pg_sf_mapping_config_path}\\pg_to_sf_mapping.csv"  
      ...
      try:
        with conn.cursor() as cursor:
          for row in reader:
            ...
            if (row['client_name'] == 'generic' and row['load_source_data']) == 'y' and row['generate_code'] == 'y':
              ...
              schema: Schema = find_or_create_schema(schema_list, pg_database, schema_name, target_database, target_schema)
              table = Table(table_name)
              cursor.execute(sql.SQL(
                  "SELECT column_name, data_type, numeric_precision, numeric_scale, udt_name FROM information_schema.columns WHERE table_schema = %s AND table_name = %s"),
...
def generate_sf_build_script(schema_list: List[Schema]):
    ...
    with open(build_script_file_path, "w") as file:
      for client in clients:
        for schema in schema_list:
          build_sql = f"-f data_ingestion\\schemas\\{schema.database}_{client}_{schema.name}\\{schema.database}_{client}_{schema.name}.sql `\n"
          file.write(build_sql) 
          for table in schema.tables:
            build_sql = f"-f data_ingestion\\schemas\\{schema.database}_{client}_{schema.name}\\tables\\{table.name}.sql `\n"
            if (table.load_method == "merge"):
              build_sql = f"{build_sql}-f data_ingestion\\schemas\\{schema.database}_{client}_{schema.name}\\tables\\stage_{table.name}.sql `\n"
            file.write(build_sql) 
...
# read_config_file()
generate_db_objects_using_config('dev', 'greyhavens')
